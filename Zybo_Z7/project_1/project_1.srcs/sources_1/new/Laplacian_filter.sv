...
// Target Devices: 
// Tool Versions: 
// Description: Module implements Laplacian spatial filter 
// 
...
module Laplacian_filter (
    ...
    localparam logic signed [7:0] LAPLACIAN_KERNEL [0:2][0:2] = '{
        '{0, 1, 0},
        '{1, -4, 1}, 
        '{0, 1, 0}
    };
       ...
       conv_result = 0;
        ...
        for(int i = 0; i < 3; i++) begin
            for(int j = 0; j < 3; j++) begin
                conv_result += $signed({1'b0, window[i][j]}) * LAPLACIAN_KERNEL[i][j];
            end
        end
